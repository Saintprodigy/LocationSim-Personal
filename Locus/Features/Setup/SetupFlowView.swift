...
struct SetupFlowView: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(supportsOnDevicePairing
                     ? "LocationSim Personal needs a one-time pairing so it can set your location. You’ll confirm a short code in Settings."
                     : "Import a pairing file from your computer — LocationSim Personal uses it securely and keeps it only on this device.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var importPairingCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            stepRow(2, "AirDrop / Share into LocationSim Personal, or copy the plist text.")
            ...
            VStack(spacing: 22) {
                ...
                VStack(spacing: 10) {
                    ...
                    Text(localDevVPNInstalled
                         ? "LocalDevVPN is installed. Open it to turn on the private tunnel LocationSim Personal needs, then come back here."
                         : "LocalDevVPN creates the private loopback tunnel LocationSim Personal uses to talk to this phone’s developer-location service.")
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    if localDevVPNInstalled {
                        ...
                        tipRow(systemImage: "power.circle.fill", title: "Connect", detail: "Tap below to open it and start the tunnel. You’ll return here automatically.")
                    } else {
                    ...
                    tipRow(systemImage: "antenna.radiowaves.left.and.right", title: "Prime offline mode", detail: "Begin one successful teleport on Wi‑Fi. LocationSim saves the local developer-service port so later sessions can start over cellular without internet or USB.")
                }
                .padding(18)
                .locusGlass(.regular, in: RoundedRectangle(cornerRadius: 20, style: .continuous))
            }

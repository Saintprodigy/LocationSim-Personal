...
struct MapHomeView: View {
    ...
    @State private var routeStops: [TripStop] = []
    ...
    private var displayedRoute: [CLLocationCoordinate2D] {
        routeCoords.isEmpty ? session.activeRoute : routeCoords
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showRouteSheet) {
            RoutePlannerSheet(
                ...
                stops: $routeStops,
                isRouting: $isRouting,
                canSave: routePathToSave.count >= 2,
    ...
    private func buildRoadRoute() {
        guard let start = routeStart ?? session.simulated ?? session.realCoordinate ?? session.pin,
        ...
        Task {
            do {
                let waypoints = [start] + routeStops.map { $0.coordinate.coordinate } + [end]
                let coords = try await RouteBuilder.roadRoute(through: waypoints, mode: session.travelMode)
                await MainActor.run {
                    routeCoords = coords
                    isRouting = false
                }
    ...
    private var routePathToSave: [CLLocationCoordinate2D] {
        if !routeCoords.isEmpty { return routeCoords }
        if !drawnPath.isEmpty { return drawnPath }
        return session.activeRoute
    }

    private var suggestedTripName: String {
        if let pinPlaceName, !pinPlaceName.isEmpty {
            return "Trip to \(pinPlaceName)"
        }
        return "Trip \(Date.now.formatted(date: .abbreviated, time: .omitted))"
    }

    private func saveCurrentRoute(named name: String) {
        session.saveTrip(
            name: name,
            coordinates: routePathToSave,
            mode: session.travelMode,
            stops: savedStops
        )
    }

    private var savedStops: [TripStop] {
        var result: [TripStop] = []
        if let routeStart {
            result.append(TripStop(name: "Start", coordinate: routeStart))
        }
        result.append(contentsOf: routeStops)
        if let routeEnd {
            result.append(TripStop(name: pinPlaceName ?? "Destination", coordinate: routeEnd))
        }
        return result
    }
    ...
    private func exportGPX() {
        ...
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("LocationSim-Route.gpx")
        do {
